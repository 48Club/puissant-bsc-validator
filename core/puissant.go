...
func RunPuissantCommitter(
	...
	pendingPool map[common.Address]types.Transactions,
	puissantPool types.PuissantPackages,
	chain *BlockChain,
	chainConfig *params.ChainConfig,
...
func (p *puissantRuntime) commitTransaction(tx *types.Transaction) {
	...
	p.env.State.Prepare(tx.Hash(), p.env.TxCount)

	txReceipt, err := commitTransaction(
		tx, p.chain, p.chainConfig, p.env.Header.Coinbase, p.env.State, p.env.GasPool, p.env.Header,
		isBundleTx && !tx.AcceptsReverting(),
		isBundleTx && pTxSeq == 0,
		p.bloomProcessor,
	)
	...
	for pSeq := 0; pSeq < len(p.pSeqID2PIDBook); pSeq++ {
		...
		for _txSeq, tx := range txsStatus {
			if included.Contains(tx.uniqueID) {
				conflict = true
				tx.error = types.PuissantErrorTxConflict
